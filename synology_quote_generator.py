...
QUOTE_FILE = "报价单.xlsx"  # 报价单文件
TEMPLATE_FILE = "报价单模板.xlsx"  # 报价单模板文件
...
class QuoteGenerator:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title(f"群晖产品报价单生成器 V{__version__}")
    ...
    def setup_ui(self):
        ...
        self.price_entry = ttk.Entry(product_frame, textvariable=self.price_var, width=20)
    ...
    def on_subcategory_selected(self, event):
        ...
        if (category and subcategory and 
            ...
            # 更新产品型号下拉框
            products = self.product_categories[category][subcategory]
            if products:
                self.product_combo['values'] = products
                self.product_combo.set('')  # 清空选择
            else:
                self.product_combo['values'] = []
                self.product_combo.set('')
                messagebox.showwarning("警告", f"{category} - {subcategory} 中没有可用的产品型号")
        else:
            self.product_combo['values'] = []
            self.product_combo.set('')
    ...
    def on_product_selected(self, event):
        ...
        if selected_product:
            try:
                # 读取产品规格
                wb = load_workbook(SPECS_FILE, read_only=True)
                sheet = wb[selected_product]
                
                # 获取主要规格信息（示例：处理器、内存、硬盘位）
                specs = []
                for row in sheet.iter_rows(min_row=3):
                    spec_name = row[0].value
                    if spec_name in ['处理器', '内存', '硬盘位']:
                        spec_value = row[2].value
                        if spec_value:
                            specs.append(f"{spec_name}: {spec_value}")
                
                # 更新规格描述
                self.current_specs = " | ".join(specs)
                
            except Exception as e:
                messagebox.showerror("错误", f"读取产品规格时出错：{str(e)}")
                self.current_specs = ""
            finally:
                wb.close()
    ...
    def add_product(self):
        ...
        try:
            ...
            messagebox.showerror("错误", "请输入有效的数量和单价")
            return
    ...
    def generate_quote(self):
        ...
        try:
            ...
            # 添加数据
            total_amount = 0
            for row, item in enumerate(self.quote_items, 5):
                ws.cell(row=row, column=1, value=item['id'])
                ws.cell(row=row, column=2, value=item['product'])
                ws.cell(row=row, column=3, value=item['specs'])
            ...
            # 添加合计行
            total_row = len(self.quote_items) + 5
            ws.merge_cells(f'A{total_row}:E{total_row}')
            ws[f'A{total_row}'] = "合计金额"
            ws[f'F{total_row}'] = f"￥{total_amount:,.2f}"
            ...
            quote_file = quote_file.replace('/', '_').replace('\\', '_').replace(':', '_').replace('*', '_') \
            ...
            messagebox.showinfo("成功", f"报价单已生成：{quote_file}")
    ...
    def update_customer_info(self, customer_name, total_amount, quote_file):
        ...
        try:
            ...
            ws.cell(row=next_row, column=7, value=quote_file)
